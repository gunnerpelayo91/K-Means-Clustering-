---
title: "Kmeans Clustering: Final Project"
...
problem statiement: given a database of just over 400,000 transactions, each labeled as 'ok', 'fraud', or 'unk' use a machine learning technique to identify 50 or so transactions that *may* by fraudulent
...
print(nOK)
names=c('Fraudulent','OK')
boxplot(log10(fraudData$Val),log10(okData$Val), names=names, horizontal = TRUE, 
        main='Dollar Value per Transaction Type', xlab = 'Transaction Value (Log10)')
grid()
...
boxplot(log10(fraudData$perItem), log10(okData$perItem), names=names, horizontal = TRUE, main='Cost per Item by Transaction Type', xlab = 'log price per item')
grid()
...
```{r}
k2 =kmeans(dfScaled, centers=2, nstart=25)
...
```{r comparePlots}
p1 = fviz_cluster(k2, geom='point', data =dfScaled) + ggtitle('k=2')
p2 = fviz_cluster(k3, geom='point', data =dfScaled) + ggtitle('k=3')
p3 = fviz_cluster(k4, geom='point', data =dfScaled) + ggtitle('k=4')
p4 = fviz_cluster(k5, geom='point', data =dfScaled) + ggtitle('k=5')
p5 = fviz_cluster(k6, geom='point', data =dfScaled) + ggtitle('k=6')
...
# compute and plot for k=1 to k=9
k.values=2:9

# extract was for 2-9 clusters
was.values = map_dbl(k.values, was)
print('silhouette values: ')
for (i in 2:9) print(paste(i," ", was.values[i]," ", was.values[i]/was.values[1]*100,"%"))
...
```{r}
final = kmeans(dfScaled,6, nstart=200)
fviz_cluster(final, geom='point', data = dfScaled) + ggtitle('Optimal Clusters')
```
...
for(i in 1:nUnkn) {
  delta1 = c(as.numeric(unkDataFrame[i,c(3,4,6)])-as.numeric(clusterCenterSave[1,]))
  delta1 = sqrt(sum(delta1))
  delta2 = c(as.numeric(unkDataFrame[i,c(3,4,6)])-as.numeric(clusterCenterSave[2,]))
  delta2 = sqrt(sum(delta1))
saveTrans[i,]=c(i, min(delta1, delta2))

}
hist(saveTrans[,2],xlim=c(0,20), main="dist to fraud", xlab='dist', freq=FALSE)
...
```{r}
sortSaveTrans = sort(saveTrans[,1], index.return=TRUE)
